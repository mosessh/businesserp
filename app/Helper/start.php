...
if (!function_exists('aiimserp_plugins')) {

    function aiimserp_plugins()
    {

        if (!session()->has('aiimserp_plugins')) {
            $plugins = \Nwidart\Modules\Facades\Module::allEnabled();

            session(['aiimserp_plugins' => array_keys($plugins)]);
        }
        return session('aiimserp_plugins');
    }
}
